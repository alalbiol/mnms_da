...
class MyResidualBlock(nn.Module):
    def __init__(self, dim, norm_layer, use_dropout, use_bias):
        ...
        res_block = [
            conv_norm_relu(dim, dim, kernel_size=1, stride=1, padding=0, norm_layer=norm_layer, bias=use_bias),
            conv_norm_relu(dim, dim, kernel_size=3, stride=1, padding=1, norm_layer=norm_layer, bias=use_bias)
        ]
        if use_dropout:
            res_block += [nn.Dropout(0.5)]

        res_block += [nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, bias=use_bias),
                      norm_layer(dim)]
...
class MyResnetGenerator(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=True,
        ...
        res_model = [conv_norm_relu(input_nc, ngf * 1, 3, 1, 1, norm_layer=norm_layer, bias=use_bias),
        ...
        for i in range(num_blocks):
            res_model += [MyResidualBlock(ngf * 4, norm_layer, use_dropout, use_bias)]
