...
def data_augmentation_selector(da_policy, img_size, crop_size, mask_reshape_method, verbose=True):
    if da_policy == "none" or da_policy is None:
        return da_policy_none(img_size, mask_reshape_method, verbose=verbose)

    elif da_policy == "random_crops":
        return da_policy_randomcrop(img_size, crop_size, mask_reshape_method, verbose=verbose)

    elif da_policy == "rotations":
        return da_policy_rotate(img_size, mask_reshape_method, verbose=verbose)

    elif da_policy == "vflips":
        return da_policy_vflip(img_size, mask_reshape_method, verbose=verbose)

    elif da_policy == "hflips":
        return da_policy_hflip(img_size, mask_reshape_method, verbose=verbose)

    elif da_policy == "elastic_transform":
        return da_policy_elastictransform(img_size, mask_reshape_method, verbose=verbose)

    elif da_policy == "grid_distortion":
        return da_policy_griddistortion(img_size, mask_reshape_method, verbose=verbose)

    elif da_policy == "shift":
        return da_policy_shift(img_size, mask_reshape_method, verbose=verbose)

    elif da_policy == "scale":
        return da_policy_scale(img_size, mask_reshape_method, verbose=verbose)

    elif da_policy == "optical_distortion":
        return da_policy_opticaldistortion(img_size, mask_reshape_method, verbose=verbose)

    elif da_policy == "coarse_dropout" or da_policy == "cutout":
        return da_policy_coarsedropout(img_size, mask_reshape_method, verbose=verbose)
...
def common_test_augmentation(img_size, mask_reshape_method, verbose=True):
...
def da_policy_none(img_size, mask_reshape_method, verbose=True):
    if verbose:
        print("Using None Data Augmentation")
    train_aug = [
    ]

    train_aug = common_test_augmentation(img_size, mask_reshape_method, verbose=verbose) + train_aug

    train_aug_img = []

    val_aug = common_test_augmentation(img_size, mask_reshape_method, verbose=verbose)

    return train_aug, train_aug_img, val_aug


def da_policy_randomcrop(img_size, crop_size, mask_reshape_method, verbose=True):  # DA_Rotate
    if verbose:
...
def da_policy_rotate(img_size, mask_reshape_method, verbose=True):  # DA_Rotate
    if verbose:
...
def da_policy_vflip(img_size, mask_reshape_method, verbose=True):
    if verbose:
...
def da_policy_hflip(img_size, mask_reshape_method, verbose=True):
    if verbose:
...
def da_policy_elastictransform(img_size, mask_reshape_method, verbose=True):
    if verbose:
...
def da_policy_griddistortion(img_size, mask_reshape_method, verbose=True):
    if verbose:
...
def da_policy_shift(img_size, mask_reshape_method, verbose=True):
    if verbose:
...
def da_policy_scale(img_size, mask_reshape_method, verbose=True):
    if verbose:
...
def da_policy_opticaldistortion(img_size, mask_reshape_method, verbose=True):
    if verbose:
...
def da_policy_coarsedropout(img_size, mask_reshape_method, verbose=True):
    if verbose:
...
def da_policy_downscale(img_size, mask_reshape_method, verbose=True):
    if verbose:
...
def mms2d_segmentation_da(img_size, mask_reshape_method, verbose=True):
    if verbose:
